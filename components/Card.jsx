  ...
  return (
    <div className="w-full leading-6 text-emerald-900 ">
      <motion.div
        ...
        className={`${border ? "border-y border-green-500" : "" } text-base z-[1] gap-3 md:gap-x-6 lg:gap-6 grid-rows-4 sm:grid-cols-2 lg:grid-rows-1 lg:grid-cols-[auto,.5fr,.5fr,.5fr,auto] items-center py-[3rem] no-underline grid place-content-start lg:place-content-center`}
      >
        ...
        <div className="lg:hidden block order-first row-span-4">
          <Image className="rounded-xl border-2 border-emerald-900" src={image} width={380} height={220} alt={"image"} />
          </div>
        <motion.div
        ...
        >
          <motion.img
            ...
            animate={isHovered ? { scale: 1, y: -70, opacity: 1 } : { scale: 0.5, opacity: 0 }}
            transition={{ type: "spring", stiffness: 260, damping: 20, delay: 0.2, duration: 0.5, ease: "easeInOut", }}
          />
        </motion.div>
        ...
        <div>
          <div className="lg:block hidden">
            <div className="px-2">
              <Flip horizontal hover={isHovered} children={<FaArrowRightLong className="mx-2" size={22} />} />
            </div>
          </div>
